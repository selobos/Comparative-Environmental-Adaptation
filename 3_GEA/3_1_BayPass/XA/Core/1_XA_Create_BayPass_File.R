...
setwd("~/Desktop/Comparative-Landscape-Genomics/3_GEA/3_1_BayPass/XA/Core") 

#Load genlight object
load("~/Desktop/Comparative-Landscape-Genomics/1_SNP_quality_control_filtering/XA/XA_gle.rdata")

#Convert genlight object to data frame
XA_df <- as.data.frame(XA_gle)
write.table(XA_df, "XA_012.txt", quote = FALSE)

#Import LONGLAT file, this should have all individuals in the same order as INPUT file
ind <- gle_order@ind.names
write.csv(ind, file = "XA_ind.csv")

pop <- gle_order@pop
write.csv(pop, file = "XA_pop.csv")

loc.names <- gle_order@loc.names
write.csv(loc.names, file = "XA_loc.names.csv")

longlat <- read.csv("XA_lonlat.csv", header = TRUE)
...
baypass_format(INPUTFILE = "XA_012.txt", LONGLAT = longlat, BAYPASSFILE = "XA_Baypass")
