...
setwd("~/Desktop/Comparative-Landscape-Genomics/3_GEA/3_1_BayPass/3_1_1_CR/3_1_1_1_Core")

#Load genlight object
load("~/Desktop/Comparative-Landscape-Genomics/1_SNP_quality_control_filtering/1_1_CR/CR_gle.rdata")

#Convert genlight object to data frame
CR_df <- as.data.frame(CR_gle)
write.table(CR_df, "CR_012.txt", quote = FALSE)

#Import LONGLAT file, this should have all individuals in the same order as INPUT file
ind <- gle_order@ind.names
write.csv(ind, file = "CR_ind.csv")

pop <- gle_order@pop
write.csv(pop, file = "CR_pop.csv")

loc.names <- gle_order@loc.names
write.csv(loc.names, file = "CR_loc.names.csv")

longlat <- read.csv("CR_lonlat.csv", header = TRUE)
...
baypass_format(INPUTFILE = "CR_012.txt", LONGLAT = longlat, BAYPASSFILE = "CR_Baypass")
