#Set directory
setwd("~Desktop/Comparative-Landscape-Genomics/3_GEA/3_1_BayPass/EI/EA")

###Calculate means for aux runs and output a csv file with only those above BF20
myList<- vector(mode = "list", length = 5)

for (i in 1:5) {
  myList[[i]]<-as.matrix(read.table(paste("ei_aux.",i,"_summary_betai.out",sep=""), header=T))
}
...
write.csv(bfmedian, file=paste0("ei_aux_median_summary_betai.out.csv"))
